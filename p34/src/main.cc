...
void dft(Complex* input, Complex* output,
    ...
    if(isRow){
        ...
        for(int i = 0; i < N; ++i){
            ...
            for(int k = 0; k < N; ++k){
                float temp = 2*PI*i*k/N;
                output[rowmult + i] =
                        output[rowmult + i] +
                        input[rowmult + k]*
                        Complex(cosf(temp), -sinf(temp));
            }
        }
    } else {
        for(int i = 0; i < N; ++i){
...
void threaded_fft(InputImage img, Complex *intermImg, int tid, bool isRow) {
    ...
    if(isRow){
        ...
        for(int row = low_bound; row <= hi_bound; ++row){
            fft(img.get_image_data(), intermImg, isRow,
        ...
        for(int col = low_bound; col <= hi_bound; ++col){
            fft(intermImg, img.get_image_data(), isRow,
...
void fft2d(InputImage inputImg){
    ...
    thread t2[parts];
    for (int i = 0; i < parts; ++i) {
        t2[i] = thread(threaded_fft,
                       ref(inputImg), intermImg, i, false);
    }

    threaded_fft(ref(inputImg), intermImg, parts, false);
...
void ifft2d(InputImage inputImg){
